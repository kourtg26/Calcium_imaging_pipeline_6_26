...
def load_ret_class_maps(ret_classes_zip: str, workdir: str) -> dict:
    ...
    if not os.path.exists(fp):
        # try any CSV that contains required columns
        cands=[os.path.join(workdir, f) for f in os.listdir(workdir) if f.lower().endswith(".csv")]
        if len(cands)==0:
            return {}
        picked=None
        for c in sorted(cands):
            try:
                ...
                continue
            cols={str(x).strip().lower() for x in d0.columns}
            if "cell_id" in cols and ("class" in cols or "group" in cols) and "animal_id" in cols:
                picked=c
                break
        if picked is None:
            return {}
...
def extract_npz_from_zip(zp: str, outdir: str) -> list:
    ...
    return sorted(list(dict.fromkeys(extracted)))
...
def load_ext2_session_csv(fp: str) -> dict | None:
    ...
    if tone_flag_col is None or freeze_col is None:
        return None
    ...
    if time_col is None:
        ...
        time_vals = df[time_col].astype(float).values
...
def classify_onset_evoked(z: np.ndarray, tone_flag: np.ndarray, tone_id: np.ndarray, time: np.ndarray,
                          ...
                          early_tones=(1,2,3), late_tones=(10,11,12),
                          min_hits_per_period: int=2) -> np.ndarray:
    """
    Returns per-cell class: EarlyOnly / Overlap / LateOnly / Neither.
    A cell must pass onset-evoked criteria in at least `min_hits_per_period`
    tone presentations within the early and/or late tone sets.
    """
    fps=infer_fps(time)
    ...
    early_set=set(early_tones)
    late_set=set(late_tones)
    min_hits=max(1, int(min_hits_per_period))
    ...
    early_resp = early >= min_hits
    late_resp = late >= min_hits
    out[early_resp & (~late_resp)]="EarlyOnly"
    out[(~early_resp) & late_resp]="LateOnly"
    out[early_resp & late_resp]="Overlap"
    return out
