#!/usr/bin/env python3
"""Plot permissive raw AUC by sex for all Ext2 LateOnly -> Ret EarlyOnly registered pairs."""
...
OUTDIR = ROOT / "matched_cell_crossed_session_analysis_2of3_3s" / "ext2_lateonly_to_ret_earlyonly_allpairs_rawauc_bySex"
...
def main() -> None:
    ...
    df = df.rename(columns={"ret_raw_auc": "ret_early_raw_auc"})
    # Use the permissive post-only Ext2 AUC when available, otherwise retain the existing strict raw AUC.
    df["ext2_raw_auc_allPairs"] = df["ext2_raw_auc_postOnly"].where(
        np.isfinite(df["ext2_raw_auc_postOnly"]), df["ext2_raw_auc"]
    )
    df["delta_ret_minus_ext2_raw"] = df["ret_early_raw_auc"] - df["ext2_raw_auc_allPairs"]
    ...
    for ax, sex in zip(axes[0], ["Female", "Male"]):
        ...
        y = sub["ret_early_raw_auc"].to_numpy(dtype=float)
        ...
        for _, row in sub.iterrows():
            ax.plot(xpos, [row["ext2_raw_auc_allPairs"], row["ret_early_raw_auc"]], color="#b8b8b8", linewidth=1.0, alpha=0.8, zorder=1)
            ax.scatter(
                xpos,
                [row["ext2_raw_auc_allPairs"], row["ret_early_raw_auc"]],
                s=34,
        ...
        ax.set_ylabel("Raw AUC (0-3 s, permissive post-only)")
        ax.set_title(f"{sex}")
    ...
    fig.tight_layout()
    fig.savefig(OUTDIR / "Ext2_LateOnly_to_Ret_EarlyOnly_allPairs_rawAUC_bySex.png", dpi=240, bbox_inches="tight")
    plt.close(fig)

    df.to_csv(OUTDIR / "Ext2_LateOnly_to_Ret_EarlyOnly_allPairs_rawAUC_values_bySex.csv", index=False)
    pd.DataFrame(summary_rows).to_csv(OUTDIR / "Ext2_LateOnly_to_Ret_EarlyOnly_allPairs_rawAUC_summary_bySex.csv", index=False)

    with open(OUTDIR / "metadata.json", "w", encoding="utf-8") as fh:
        json.dump(
            {
                "comparison": "Ext2 LateOnly -> Ret EarlyOnly",
                "metric": "raw_auc_0to3_postOnly_permissive",
                "source_pairs": str(PAIR_FILE),
                "notes": [
                    "Includes all explicit registered Ext2 LateOnly -> Ret EarlyOnly pairs, not only the strict valid-AUC subset.",
                    ...
                    "Retrieval raw AUC comes from the explicit pair table and is available for all rows here.",
                ],
            },
            fh,
            indent=2,
