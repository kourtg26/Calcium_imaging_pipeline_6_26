    ...
    for sess, paths, kind in sessions:
        ...
        for pth in paths:
            ...
            for c in CLASSES:
                ...
                pooled_cells[c].append((t_axis, cm))
                pooled_animals[c].append((t_axis, np.nanmean(cm, axis=0)))
        ...
        for c in CLASSES:
            if len(pooled_animals[c])>0:
                A=np.vstack([np.interp(COMMON, t_ax, a) for (t_ax, a) in pooled_animals[c]])
        ...
        def overlay_plot(weight, pooled_dict, tag):
            ...
            for c in CLASSES:
                ...
                if weight=="cell":
                    M=np.vstack([
                        np.vstack([np.interp(COMMON, t_ax, cm[i]) for i in range(cm.shape[0])])
                        for (t_ax, cm) in pooled_dict[c]
                    ])
                    mean=np.nanmean(M, axis=0)
                    sem=np.nanstd(M, axis=0, ddof=1)/np.sqrt(M.shape[0]) if M.shape[0]>1 else np.full_like(mean,np.nan)
                    label=f"{c} (cells={M.shape[0]})"
                else:
                    A=np.vstack([np.interp(COMMON, t_ax, a) for (t_ax, a) in pooled_dict[c]])
                    mean=np.nanmean(A, axis=0)
                    sem=np.nanstd(A, axis=0, ddof=1)/np.sqrt(A.shape[0]) if A.shape[0]>1 else np.full_like(mean,np.nan)
