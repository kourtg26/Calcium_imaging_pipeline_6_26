#!/usr/bin/env python3
"""
ext1_ext2_transition_pipeline.py
...
2) Merge Ext1 and Ext2 classifications by (animal_id, cell_id) assuming registration preserves cell_id strings,
   compute transition matrices and export Sankey/alluvial plots (SVG/PDF/PNG) + CSV tables.

Inputs
------
- Ext1 classification zip (already produced): ext1_classes_proportions_only.zip
...
- Ext1toExt2_transition_counts_cellWeighted.csv
- Ext1toExt2_transition_props_animalWeighted.csv
- Sankey/alluvial plots (overall + by sex, cell-weighted and animal-weighted)

Usage
-----
python ext1_ext2_transition_pipeline.py \
...
def sankey_plot_html(link_df: pd.DataFrame, value_col: str, title: str, out_html: str, left_label="Ext1", right_label="Ext2"):
    if go is None:
        return
    df = link_df.copy()
    df = df[df[value_col] > 0].copy()
...
def _best_ext2_id_alignment(ext1_df: pd.DataFrame, ext2_df: pd.DataFrame):
    """
    Try identity and scaled (/10) ID alignments for ext2 and pick the one with
    maximal overlap to ext1 IDs. Returns:
      aligned_ext2_df, strategy_name, raw_overlap, best_overlap
    """
    d1 = ext1_df.copy()
    d2 = ext2_df.copy()
    ...
    # Candidate 2: divide-by-10 only for values divisible by 10
    cands.append(
        (
            "div10_when_divisible",
            d2["_id_int"].map(lambda v: (int(v) // 10) if pd.notna(v) and int(v) % 10 == 0 else (int(v) if pd.notna(v) else np.nan)),
        )
    )

    best_name = "identity"
    best_series = d2["_id_int"]
...
def compute_transitions(ext1: dict, ext2: dict):
    animals = sorted(set(ext1.keys()).intersection(ext2.keys()))
    rows=[]
    mats={}
    diagnostics=[]
    for a in animals:
        d1=ext1[a].rename(columns={"class":"ext1"})
        d2_src=ext2[a].rename(columns={"class":"ext2"})
        d2_aligned, strategy, raw_ov, best_ov = _best_ext2_id_alignment(
            d1.rename(columns={"ext1":"class"}), d2_src.rename(columns={"ext2":"class"})
        )
        d2=d2_aligned.rename(columns={"class":"ext2"})
        m=d1.merge(d2, on="cell_id", how="inner")
        if m.empty:
            continue
        ct=pd.crosstab(m["ext1"], m["ext2"]).reindex(index=VALID, columns=VALID, fill_value=0)
        mats[a]=ct
        diagnostics.append({
            "animal_id": a,
            "sex": sex_of(a),
            ...
            "n_ext1_cells": int(d1["cell_id"].nunique()),
            "n_ext2_cells": int(d2_src["cell_id"].nunique()),
...
def save_tone_responsive_retention(prefix: str, counts: pd.DataFrame, out_dir: str):
    ...
    for aid, sub in counts.groupby("animal"):
        ...
        rows.append({
            "animal": aid,
            "sex": sx,
            "ext1_responsive_total": src_total,
            "ext2_responsive_from_ext1_resp": tgt_retained,
            "ext2_nonresponsive_from_ext1_resp": tgt_non,
    ...
    for metric, fname in [
        ...
        if len(m) >= 2 and len(f) >= 2:
            ...
            row = {
                ...
                "perm_p": perm_p_mean_diff(m, f, n_perm=10000, seed=99),
                "n_male": int(len(m)),
                "n_female": int(len(f)),
            }
        else:
            row = {"metric": metric, "male_mean": np.nan, "female_mean": np.nan, "diff_m_f": np.nan, "welch_t": np.nan, "welch_p": np.nan, "perm_p": np.nan, "n_male": len(m), "n_female": len(f)}
        pd.DataFrame([row]).to_csv(os.path.join(out_dir, fname), index=False)
    return out

def save_source_specific_outputs(prefix: str, counts: pd.DataFrame, props_all: pd.DataFrame, out_dir: str, left_label="Ext1", right_label="Ext2"):
    for src in ["EarlyOnly", "LateOnly"]:
...
def main():
    ...
    ap.add_argument("--ext1_zip", required=True, help="Ext1 classes zip (e.g., /mnt/data/ext1_classes_proportions_only.zip)")
    ...
    ap.add_argument("--work_dir", default="/mnt/data/_ext1_ext2_transition_work")
    ...
    # 2) Load Ext1 from provided zip + Ext2 from newly created zip
    ext1, _ = load_class_tables_from_zip(args.ext1_zip, "ext1", work_dir)
    ext2, _ = load_class_tables_from_zip(ext2_zip_out, "ext2", work_dir)

    trans, mats, diag = compute_transitions(ext1, ext2)
    if trans.empty:
        raise RuntimeError("No registered cells found after merging Ext1 and Ext2 by cell_id.")
    ...
    save_source_specific_outputs(prefix, counts_by_animal, props_by_animal, out_dir, left_label="Ext1", right_label="Ext2")
    ...
    alluvial_plot(cell_counts, "count", "Ext1 → Ext2 transitions (cell-weighted counts)", os.path.join(out_dir, f"{prefix}_alluvial_cellWeighted"), left_label="Ext1", right_label="Ext2")
    ap = animal_props.copy()
    ap["pct"] = 100 * ap["prop"]
    alluvial_plot(ap, "pct", "Ext1 → Ext2 transitions (animal-weighted mean %)", os.path.join(out_dir, f"{prefix}_alluvial_animalWeighted_meanPct"), left_label="Ext1", right_label="Ext2")
    for s in ["Female", "Male"]:
        if s in bysex_counts:
            alluvial_plot(bysex_counts[s], "count", f"Ext1 → Ext2 transitions (cell-weighted) — {s}", os.path.join(out_dir, f"{prefix}_alluvial_cellWeighted_{s}"), left_label="Ext1", right_label="Ext2")
        if s in bysex_props:
            bp = bysex_props[s].copy()
            bp["pct"] = 100 * bp["prop"]
            alluvial_plot(bp, "pct", f"Ext1 → Ext2 transitions (animal-weighted mean %) — {s}", os.path.join(out_dir, f"{prefix}_alluvial_animalWeighted_meanPct_{s}"), left_label="Ext1", right_label="Ext2")

    # 4) metadata + bundle
    meta = {
        "timestamp_utc": datetime.datetime.utcnow().isoformat(timespec="seconds")+"Z",
        "ext1_zip": args.ext1_zip,
        ...
        "n_animals_overlap": int(len(set(ext1.keys()).intersection(ext2.keys()))),
        "unknown_sex_animals": sorted([a for a in set(ext1.keys()).intersection(ext2.keys()) if sex_of(a)=="Unknown"]),
    ...
    bundle = os.path.join(out_dir, "ext1_to_ext2_transitions_alluvial_bundle.zip")
    bundle_files = [ext2_zip_out]
    for fn in os.listdir(out_dir):
        if fn.startswith("Ext1toExt2_") and not fn.endswith(".zip"):
