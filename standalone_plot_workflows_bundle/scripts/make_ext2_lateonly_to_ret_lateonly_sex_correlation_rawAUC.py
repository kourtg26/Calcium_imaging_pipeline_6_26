#!/usr/bin/env python3
"""Plot Ext2 LateOnly -> Ret LateOnly matched-cell raw AUC correlations split by sex."""
...
OUTDIR = ROOT / "matched_cell_crossed_session_analysis_2of3_3s" / "ext2_lateonly_to_ret_lateonly_sex_correlation_rawAUC"
PAIR_FILE = (
    ROOT
...
def plot_panel(ax, df: pd.DataFrame, title: str, color: str, level: str) -> dict[str, float]:
    ...
    ax.set_ylabel("Ret LateOnly raw AUC (0-3 s)", fontsize=9)
    ann = (
        f"n={stats_out['n']}\n"
...
def main() -> None:
    ...
    fig.suptitle("Ext2 LateOnly -> Ret LateOnly Raw AUC Correlations by Sex", fontsize=13, y=0.995)
    fig.tight_layout()
    png_path = OUTDIR / "Ext2_LateOnly_to_Ret_LateOnly_rawAUC_correlations_bySex.png"
    fig.savefig(png_path, dpi=220, bbox_inches="tight")
    plt.close(fig)

    pairs_csv = OUTDIR / "Ext2_LateOnly_to_Ret_LateOnly_rawAUC_pairs_bySex.csv"
    stats_csv = OUTDIR / "Ext2_LateOnly_to_Ret_LateOnly_rawAUC_correlation_stats_bySex.csv"
