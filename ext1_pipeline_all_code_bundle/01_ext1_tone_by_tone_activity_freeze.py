...
Uses:
  - per-animal per-cell classes from 00_* (ext1_classes directory or zip)
  - Ext1 z-scored NPZs

Outputs:
  - Ext1_toneByTone_activity_freezing_perAnimal.csv
  - Ext1_toneByTone_summary_meanSEM.csv
  - Ext1_toneByTone_activityLines_freezeBars_{ALL,Male,Female}.png
  - ext1_toneByTone_activity_freeze_bundle.zip
"""
...
from freezing_pipeline_all_code_bundle.utils_freezing import (
    extract_npz_parts, load_ext1_npz, standardize_class_label, infer_fps
)
...
def plot_tone_by_tone(summary_df, sex_label, out_png):
    ...
    ax1.set_title(f"Ext1 tone-block activity (first 3s) with freezing ({sex_label})\nAnimal-weighted mean ± SEM")
    plt.tight_layout()
...
def main():
    ...
    ext1_zip = cfg["paths"].get("ext1_classes_zip")
    if ext1_zip and not glob.glob(os.path.join(class_dir, "*_ext1_onset_evoked_cell_classes.csv")):
        with zipfile.ZipFile(ext1_zip, "r") as zf:
            zf.extractall(class_dir)

    class_files = sorted(glob.glob(os.path.join(class_dir, "*_ext1_onset_evoked_cell_classes.csv")))
    class_map = {}
    ...
    ext1_npz = extract_npz_parts(cfg["paths"]["ext1_zscored_npz_parts"], out_dir)

    tone_cfg = (cfg.get("tone_by_tone", {}) or {}).get("Ext1", {})
    trace_cfg = (cfg.get("tone_trace", {}) or {}).get("Ext1", {})
    ...
    for npz_path in ext1_npz:
        ...
        animal_raw = os.path.basename(npz_path).replace("_ext1_zscored_traces.npz", "")
        ...
        if len(epochs_all) == 0:
            continue
        # keep config selection only if needed elsewhere
        ...
        for block_name, tone_orders in blocks.items():
            ...
            for e in keep:
                ...
                # mean z in first 3s after onset
                onset = e["onset_idx"]
        ...
        for label, group_epochs in [("first", first_epochs), ("last", last_epochs)]:
            ...
            if traces is None:
                continue
            # traces: n_events x T x C
            t_axis = np.arange(-preF, postF + 1) / fps
            for cls_name, mask in [("EarlyOnly", early_mask), ("LateOnly", late_mask)]:
                if not np.any(mask):
                    continue
                cls_tr = traces[:, :, mask]  # n_events x T x C
                ev_mean = np.nanmean(cls_tr, axis=0)  # T x C
                cell_mean = np.nanmean(ev_mean, axis=1)  # T
                trace_rows.append({
                    "animal_id": animal,
    ...
    tone_csv = os.path.join(out_dir, "Ext1_toneBlock_activity_freezing_perAnimal.csv")
    ...
    summary_csv = os.path.join(out_dir, "Ext1_toneBlock_summary_meanSEM.csv")
    summary.to_csv(summary_csv, index=False)

    png_all = os.path.join(out_dir, "Ext1_toneBlock_activityLines_freezeBars_ALL.png")
    png_m = os.path.join(out_dir, "Ext1_toneBlock_activityLines_freezeBars_Male.png")
    png_f = os.path.join(out_dir, "Ext1_toneBlock_activityLines_freezeBars_Female.png")
    ...
    if trace_rows:
        ...
        # aggregate per class/group across animals
        for cls_name in ["EarlyOnly", "LateOnly"]:
            for label in ["first", "last"]:
                ...
                out_csv = os.path.join(out_dir, f"Ext1_toneTrace_{cls_name}_{label}3_meanSEM.csv")
                pd.DataFrame({"time_s": common_t, "mean": mean, "sem": sem}).to_csv(out_csv, index=False)

            # plot first vs last for this class
            ...
            if first and last:
                ...
                ax.set_title(f"Ext1 {cls_name}: first 3 vs last 3 tones (Δz)")
                ax.legend(frameon=False)
                plt.tight_layout()
                out_png = os.path.join(out_dir, f"Ext1_toneTrace_{cls_name}_first3_vs_last3.png")
                plt.savefig(out_png, dpi=300)
                plt.close()

    out_zip = os.path.join(out_dir, "ext1_toneBlock_activity_freeze_bundle.zip")
