#!/usr/bin/env python3
"""Create a clean 3-session montage for the top-of-FOV cells in animal 5.

Uses the highest trio that is present in all three sessions:
    C001, C015, C087

For each session, pick the frame where all three are simultaneously active at
z >= 0.2 and maximize the summed z across the trio. Draw outlines only.
"""
...
OUT_DIR = DATA_DIR / "representative_cell_images_animal5" / "topFOV_montage_thr02"
...
def pick_joint_frame(trace_matrix: np.ndarray):
...
def draw_panel(movie_path: Path, movie_frame_idx: int, outlines, off_x: int, off_y: int, class_map, session_label):
    ...
    draw.rectangle((6, 6, 140, 22), fill=(0, 0, 0))
    draw.text((10, 10), f"{session_label}  z>={THRESHOLD}", fill=(255, 255, 255))
    return rgb
...
def main():
    ...
    for sess in SESSIONS:
        ...
        csv_frame_idx, active_count, exact_match = pick_joint_frame(trace_matrix)
        values = trace_matrix[csv_frame_idx].tolist()
        ...
        out_path = OUT_DIR / f"{sess}_topFOV_thr02_frame{movie_frame_idx:05d}.png"
        panel.save(out_path)
        panels.append(panel)
        summary_rows.append({
            "session": sess,
            "csv_frame_index": int(csv_frame_idx),
            "movie_frame_index": movie_frame_idx,
    ...
    if panels:
        ...
        montage.save(OUT_DIR / "animal5_topFOV_thr02_montage.png")

    with (OUT_DIR / "animal5_topFOV_thr02_summary.csv").open("w", newline="") as f:
