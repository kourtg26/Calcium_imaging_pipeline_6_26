#!/usr/bin/env python3
"""Plot Ext2 EarlyOnly -> Ret EarlyOnly matched-cell AUC correlations split by sex."""
...
OUTDIR = ROOT / "matched_cell_crossed_session_analysis_2of3_3s" / "ext2_earlyonly_to_ret_earlyonly_sex_correlation"
PAIR_FILE = (
    ROOT
    / "ext2_ret_transition_output_updatedCriteria_2of3_3s/registered_pair_auc_validation/Ext2_EarlyOnly_to_Ret_EarlyOnly_registered_pairs_withAUC.csv"
)
...
def plot_panel(ax, df: pd.DataFrame, title: str, color: str, level: str) -> dict[str, float]:
    ...
    ax.set_xlabel("Ext2 EarlyOnly AUC (0-3 s, baseline-corrected)", fontsize=9)
    ax.set_ylabel("Ret EarlyOnly AUC (0-3 s, baseline-corrected)", fontsize=9)
    ann = (
        f"n={stats_out['n']}\n"
...
def main() -> None:
    ...
    sub["comparison"] = "Ext2 EarlyOnly -> Ret EarlyOnly"
    sub["source_session"] = "Ext2"
    sub["source_class"] = "EarlyOnly"
    sub["target_session"] = "Ret"
    sub["target_class"] = "EarlyOnly"
    ...
    fig.suptitle("Ext2 EarlyOnly -> Ret EarlyOnly AUC Correlations by Sex", fontsize=13, y=0.995)
    fig.tight_layout()
    png_path = OUTDIR / "Ext2_EarlyOnly_to_Ret_EarlyOnly_AUC_correlations_bySex.png"
    fig.savefig(png_path, dpi=220, bbox_inches="tight")
    plt.close(fig)

    cell_csv = OUTDIR / "Ext2_EarlyOnly_to_Ret_EarlyOnly_pairs_bySex.csv"
    stats_csv = OUTDIR / "Ext2_EarlyOnly_to_Ret_EarlyOnly_correlation_stats_bySex.csv"
    sub.sort_values(["sex", "animal_id", "cell_id"]).to_csv(cell_csv, index=False)
    pd.DataFrame(stats_rows).to_csv(stats_csv, index=False)

    meta = {
        "comparison": "Ext2 EarlyOnly -> Ret EarlyOnly",
